...
class BinanceFuturesClient:
    ...
    async def place_limit_entry(
        ...
        if self.dry_run:
            order_id = f"DRYRUN_LIMIT_{int(datetime.now(pytz.UTC).timestamp() * 1000)}"
            self.logger.info(f"[DRY RUN] Limit Entry: {direction} {quantity} @ ${price}")
            return {'orderId': order_id, 'status': 'DRY_RUN', 'updateTime': int(datetime.now(pytz.UTC).timestamp() * 1000)}
