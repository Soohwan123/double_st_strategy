#!/usr/bin/env python3
"""FVG Retest Live Trading — XRPUSDT 15m"""
...
async def ipc_subscriber_task(strategy):
    """price_feed 의 ZMQ PUB 에서 XRPUSDT 시세 수신 → strategy 에 전달"""
    subscriber = IPCSubscriber(
        symbol="XRPUSDT",
        on_kline_15m=strategy.on_candle_close,
        on_kline_1h=strategy.on_htf_kline,
...
async def position_sync_task(strategy: FvgStrategy, interval: int = 30):
    ...
    while True:
        try:
            ...
            # DRY 모드: binance 조회 불필요 (모든 게 로컬 시뮬레이션, on_tick에서 처리)
