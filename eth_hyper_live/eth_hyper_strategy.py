#!/usr/bin/env python3
"""
ETH Hyper Scalper Strategy (ETHUSDT 15m, EMA20 Retest)
EMA 정배열/역배열 + ADX + Retest 기반 추세추종 전략

핵심 로직:
1. 15분봉 마감 시 신호 체크 → 시장가 진입
2. 동적 레버리지 (손절거리 기반)
3. 동적 익절 (ATR 기반, 롱/숏 별도 배수)
4. 틱데이터로 TP/SL 체결 감지
5. 포지션 종료 시 자본금 업데이트
"""
...
class EthHyperStrategy:
    """ETH Hyper Scalper 전략 클래스"""
    ...
    def __init__(
        ...
        self.position = EthHyperPositionState()
    ...
    def _get_param(self, key: str, default=None):
        return self.dynamic_config.get(key, ETH_HYPER_DEFAULT_PARAMS.get(key, default))
    ...
    async def initialize(self):
        self.logger.info("=" * 60)
        self.logger.info("ETH Hyper Scalper Strategy 초기화")
        ...
        self.candle_manager = CandleDataManager(
            max_candles=500,
            ema_fast=self._get_param('EMA_FAST', 20),
            ema_mid=self._get_param('EMA_MID', 100),
            ema_slow=self._get_param('EMA_SLOW', 200),
            adx_length=self._get_param('ADX_LENGTH', 14),
            atr_length=self._get_param('ATR_LENGTH', 10),
            retest_lookback=self._get_param('RETEST_LOOKBACK', 15),
            sl_lookback=self._get_param('SL_LOOKBACK', 50),
            logger=self.logger
        )
    ...
    async def load_historical_data(self):
        ...
        try:
            ...
            if indicators:
                self.logger.info(
                    f"현재 지표: EMA{self._get_param('EMA_FAST', 20)}={indicators['ema_fast']:.2f}, "
        ...
        try:
            ...
            with open(self.indicators_path, 'w', newline='') as f:
                ...
                for _, row in all_indicators.iterrows():
                    adx_threshold = self._get_param('ADX_THRESHOLD', 40.0)
                    long_signal = (
    ...
    def calculate_stop_loss(self, entry_price: float, direction: str) -> float:
        ...
        max_sl_distance = self._get_param('MAX_SL_DISTANCE', 0.035)
        ...
        # 최대 거리 캡 적용 (백테스트와 동일 — 0.1% fallback 없음)
    ...
    def calculate_take_profit(self, entry_price: float, direction: str, atr: float) -> float:
        """익절가 계산 (ATR 기반 + 수수료 보전, 롱/숏 별도 배수)"""
        taker_fee = self._get_param('TAKER_FEE', 0.0005)
        ...
        if direction == 'LONG':
            tp_mult = self._get_param('TP_ATR_MULT_LONG', 10.0)
            return entry_price + atr * tp_mult + fee_offset
        else:
            tp_mult = self._get_param('TP_ATR_MULT_SHORT', 2.0)
            return entry_price - atr * tp_mult - fee_offset
    ...
    async def execute_entry(self, direction: str):
        ...
        self.logger.info(f"포지션 크기: {entry_size:.6f} ETH (${position_value:.2f})")
        self.logger.info(f"{'='*50}")
    ...
    async def _get_actual_pnl(self, exit_type: str) -> float:
        """바이낸스 API에서 실제 체결 PnL 조회 (진입수수료 + 청산수수료 포함)"""
    ...
    async def _sync_capital_with_binance(self):
        ...
        try:
            ...
            usable_balance = actual_balance * 0.90
    ...
    async def on_candle_close(self, kline: Dict):
        ...
        self.logger.info(
            ...
            f"O:{float(kline['o']):.2f} H:{float(kline['h']):.2f} "
            f"L:{float(kline['l']):.2f} C:{float(kline['c']):.2f}"
        )

        indicators = self.candle_manager.get_latest_indicators()
        if indicators and not pd.isna(indicators['adx']):
            self.logger.info(
                f"지표: ADX={indicators['adx']:.1f}, ATR={indicators['atr']:.2f}, "
