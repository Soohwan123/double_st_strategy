#!/usr/bin/env python3
"""Breakout Live Trading — SOLUSDT 5m bt_02 v4 best (Hyper 부계정)"""
...
async def ipc_subscriber_task(strategy):
    """price_feed 의 ZMQ PUB 에서 SOLUSDT 시세 수신.
    bt_02 v4 best: kline_5m + trade. HTF 안 씀.
    """
        ...
        try:
            ...
            if strategy.position.has_position() and pos_info is None:
                ...
                if exit_type == 'TP' and exit_price:
                    ...
                    logger.warning("TP/SL 확인 불가 — actual PnL 조회 (entry_order_id + startTime, SOLUSDT 만)")
                    pnl_data = None
...
async def main():
    logger = log_handler
    logger.info("=" * 70)
    logger.info(f"Breakout Live Trading 시작 ({SYMBOL}) — bt_02 v4 best")
    logger.info("=" * 70)
    try:
        Config.validate()
