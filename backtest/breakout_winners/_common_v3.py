"""
Trendlines with Breaks (LuxAlgo) v3 Swing-SL — common backtest with trade logging.

Exit (v3):
  - LONG  SL = min(last_swing_low, ep - atr*MIN_SL_ATR)  (더 먼 SL 보수)
  - SHORT SL = max(last_swing_high, ep + atr*MIN_SL_ATR)
  - TP = entry ± RR × SL_dist
...
def run_backtest(symbol, tf, length, mult, min_sl_atr, rr, risk_per_trade):
    ...
    swing_low_init = False; swing_high_init = False
    ...
    for i in range(2*L + 1, n):
        ...
        if is_ph:
            ...
            last_swing_high = ph_val; swing_high_init = True
        elif upper_init:
            upper -= slope_ph
        if is_pl:
            lower = pl_val; slope_pl = slope_now; lower_init = True
            last_swing_low = pl_val; swing_low_init = True
        elif lower_init:
            lower += slope_pl
        ...
        if position == 0 and upper_init and lower_init and swing_low_init and swing_high_init:
            if up_break and not dn_break:
                ep = c[i]
                atr_now = atr[i]
                if np.isnan(atr_now) or atr_now <= 0: continue
                sl_from_swing = last_swing_low
                sl_from_atr = ep - atr_now * min_sl_atr
                sl_edge = min(sl_from_swing, sl_from_atr)
                ...
                atr_now = atr[i]
                if np.isnan(atr_now) or atr_now <= 0: continue
                sl_from_swing = last_swing_high
                sl_from_atr = ep + atr_now * min_sl_atr
                sl_edge = max(sl_from_swing, sl_from_atr)
                if sl_edge <= ep: continue
                sl_dist = sl_edge - ep
                tp_edge = ep - rr * sl_dist
                sl_pct = sl_dist / ep
