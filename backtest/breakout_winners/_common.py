"""
Trendlines with Breaks (LuxAlgo) breakout strategy — common backtest with trade logging.

검수:
  - 시그널: c[i] close 확정 후 같은 bar 시장가 entry (taker fee). 보수적이려면 next bar open 진입.
  - Pivot: ta.pivothigh(L,L) 동등. bar (i-L) 가 pivot — i 시점에 확정 가능 (look-ahead 없음).
  - Trendline slope: ATR(L) at bar pi / L * mult — bar pi 까지 데이터만.
  - Exit: 진입봉 다음 (i+1) bar 부터 OHLC 로 SL/TP/LIQ 체크. 진입봉 안 SL/TP 무시 (보수).
  - Same-bar SL+TP: SL 우선 (LIQ > SL > TP).
  - Fee: entry market=TAKER, SL=TAKER, TP=MAKER (limit), END=TAKER.
"""
import os
import numpy as np
import pandas as pd
...
def run_backtest(symbol, tf, length, mult, sl_atr_mult, rr, risk_per_trade):
