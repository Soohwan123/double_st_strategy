...
# ADX 설정
ADX_LENGTH = 14
<<<<<<< Updated upstream:backtest_hyper_scalper_v2_usdc_test.py
ADX_THRESHOLD = 30.0
=======
ADX_THRESHOLD = 35.0
>>>>>>> Stashed changes:backtest/backtest_hyper_scalper_v2_usdc_test.py
...
# ATR 설정
ATR_LENGTH = 10
TP_ATR_MULT_LONG = 4.1   # 롱 익절 ATR 배수
TP_ATR_MULT_SHORT = 4.1  # 숏 익절 ATR 배수
...
# ============================================
<<<<<<< Updated upstream:backtest_hyper_scalper_v2_usdc_test.py
START_DATE = '2026-01-10 02:30:00'      # 지표 계산 시작 (500봉 warmup)
TRADE_START_DATE = '2026-01-15 07:31:00'  # 실제 거래 시작 (KST 16:31 = UTC 07:31)
END_DATE = '2026-01-20 23:00:00'        # 백테스트 종료
=======
START_DATE = '2024-01-01 02:30:00'      # 지표 계산 시작 (500봉 warmup)
TRADE_START_DATE = '2024-02-05 07:31:00'  # 실제 거래 시작 (KST 16:31 = UTC 07:31)
END_DATE = '2026-02-22 23:00:00'        # 백테스트 종료
>>>>>>> Stashed changes:backtest/backtest_hyper_scalper_v2_usdc_test.py
...
def calculate_ema(series: pd.Series, length: int) -> pd.Series:
    """TradingView EMA 계산 (기존 방식 - 참고용)"""
    return series.ewm(span=length, adjust=False).mean()


def calculate_ema_incremental(series: pd.Series, length: int, initial_window: int = 500) -> pd.Series:
    """
    증분 방식 EMA 계산 (라이브 트레이딩 시뮬레이션)
    - 처음 initial_window개: ewm()으로 계산
    - 이후: 증분 계산 (이전 EMA 값 기반)

    라이브에서는 500개 FIFO + 매번 전체 재계산하지만,
    증분 방식은 이전 EMA를 저장하고 새 값만 계산하므로 동일한 결과.
    """
    ...
    if len(series) >= initial_window:
        # 처음 500개: ewm으로 계산 (라이브 초기 로드 시뮬레이션)
        initial_ema = series.iloc[:initial_window].ewm(span=length, adjust=False).mean()
        result[:initial_window] = initial_ema.values

        # 이후: 증분 계산 (라이브의 웹소켓 업데이트 시뮬레이션)
        for i in range(initial_window, len(series)):
            result[i] = alpha * series.iloc[i] + (1 - alpha) * result[i - 1]
    else:
        # 500개 미만이면 전체 ewm
        result[:] = series.ewm(span=length, adjust=False).mean().values
...
class HyperScalperBacktester:
    ...
    def _calculate_indicators(self):
        """지표 계산 (증분 방식 - 라이브와 동일)"""
        # EMA - 증분 방식 (처음 500개: ewm, 이후: 증분 계산)
        self.df['ema_fast'] = calculate_ema_incremental(self.df['close'], EMA_FAST, initial_window=500)
        self.df['ema_mid'] = calculate_ema_incremental(self.df['close'], EMA_MID, initial_window=500)
        self.df['ema_slow'] = calculate_ema_incremental(self.df['close'], EMA_SLOW, initial_window=500)
    ...
    def run(self):
        ...
        print(f"Trade start: {TRADE_START_DATE} UTC (KST 16:31)")
        print("-" * 50)
        ...
        for idx in range(1, len(self.df)):
            ...
            # 포지션이 없을 때 진입 확인
                ...
                # TRADE_START_DATE 이후에만 거래 시작
                if row['timestamp'] < pd.Timestamp(TRADE_START_DATE):
                    continue
...
if __name__ == "__main__":
    ...
    # 결과 저장 (증분 방식 테스트용)
    backtester.save_trades('trades_hyper_scalper_v2_usdc_incremental.csv')
    backtester.save_data_with_indicators('data_with_hyper_scalper_v2_usdc_incremental.csv')
