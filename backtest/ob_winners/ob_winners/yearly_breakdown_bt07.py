"""bt_07 ETH 5m OB SWAP 연도별."""
import sys; sys.path.insert(0, '..')
import _common_swap as C
from _common_swap import run_backtest

PARAMS = dict(impulse_lookback=7, impulse_min_pct=0.016, sl_buffer_pct=0.00005,
              rr=1.2, max_wait=55, risk_per_trade=0.035, use_htf=True)
...
C.START='2020-01-06'; C.END='2026-04-23'
trades_all, cap_all, mdd_all = run_backtest(symbol='ETHUSDT', tf='5m', **PARAMS)
ret_all = (cap_all/C.INITIAL_CAPITAL-1)*100
wins_all = sum(1 for t in trades_all if t['pnl']>0)
print(f"\n{'='*78}\n  bt_07 ETH 5m OB SWAP v6")
...
for yname, ys, ye in YEARS:
    C.START=ys; C.END=ye
    trades, cap, mdd = run_backtest(symbol='ETHUSDT', tf='5m', **PARAMS)
