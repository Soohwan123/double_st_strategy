"""
Hyper Scalper Strategy Backtester V2 + DI Filter
EMA25/100/200 + ADX + DI Filter + Retest (LONG & SHORT)
손절: 최근 N봉 최저가(LONG) / 최고가(SHORT)

LONG 진입 조건:
1. bullTrend: close > EMA200 AND EMA25 > EMA100 > EMA200 (정배열)
2. strongTrend: ADX >= 30
3. diLongFilter: +DI > -DI AND (+DI - -DI) >= 5  ★ 추가된 조건
4. hadLowBelowFast: 최근 5봉 내 저가가 EMA25 아래였던 적 있음
5. reclaimLong: 현재 종가 > EMA25
...
2. strongTrend: ADX >= 30
3. diShortFilter: -DI > +DI AND (-DI - +DI) >= 5  ★ 추가된 조건
4. hadHighAboveFast: 최근 5봉 내 고가가 EMA25 위였던 적 있음
5. reclaimShort: 현재 종가 < EMA25
- 익절: 진입가 - ATR * TP_ATR_MULT
- 손절: 최근 N봉 최고가 (최대 3% 캡)
"""
...
# DI 필터 설정 (진입 조건 추가)
# LONG: +DI > -DI 이고 +DI - -DI >= DI_DIFF_THRESHOLD
# SHORT: -DI > +DI 이고 -DI - +DI >= DI_DIFF_THRESHOLD
...
def calculate_dmi(high: pd.Series, low: pd.Series, close: pd.Series, length: int) -> tuple:
    """
    TradingView DMI 계산 (+DI, -DI, ADX 반환)
    ta.dmi(adxLen, adxLen) 와 동일
...
class HyperScalperBacktester:
    ...
    def _calculate_indicators(self):
        ...
        # DMI (+DI, -DI, ADX)
        plus_di, minus_di, adx = calculate_dmi(self.df['high'], self.df['low'], self.df['close'], ADX_LENGTH)
        ...
        # DI 필터 조건
        # LONG: +DI > -DI 이고 차이가 DI_DIFF_THRESHOLD 이상
        self.df['di_long_filter'] = (
            (self.df['plus_di'] > self.df['minus_di']) &
            ((self.df['plus_di'] - self.df['minus_di']) >= DI_DIFF_THRESHOLD)
        )
        # SHORT: -DI > +DI 이고 차이가 DI_DIFF_THRESHOLD 이상
        self.df['di_short_filter'] = (
            (self.df['minus_di'] > self.df['plus_di']) &
            ((self.df['minus_di'] - self.df['plus_di']) >= DI_DIFF_THRESHOLD)
        )
        ...
        # Long signal (DI 필터 추가)
        self.df['long_signal'] = (
            self.df['bull_trend'] &
            self.df['strong_trend'] &
            self.df['di_long_filter'] &  # +DI > -DI + 5
            self.df['had_low_below_fast'] &
            self.df['reclaim_long']
        )

        # Short signal (DI 필터 추가)
        self.df['short_signal'] = (
            self.df['bear_trend'] &
            self.df['strong_trend'] &
            self.df['di_short_filter'] &  # -DI > +DI + 5
    ...
    def save_data_with_indicators(self, filename: str):
        ...
        cols = ['timestamp', 'open', 'high', 'low', 'close', 'volume',
                ...
                'plus_di', 'minus_di', 'adx', 'atr',
                'di_long_filter', 'di_short_filter',
...
if __name__ == "__main__":
    ...
    # 결과 저장
    backtester.save_trades('trades_hyper_scalper_v2_di.csv')
    backtester.save_data_with_indicators('data_with_hyper_scalper_v2_di.csv')
