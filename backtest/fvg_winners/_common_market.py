...
def run_backtest(symbol, tf, version,
    ...
    def process_sl_exit(bar_i, exit_price):
        ...
        entry_fee = entry_price * sz_remain * TAKER_FEE
    ...
    for i in range(2, n):
            ...
            if is_partial:
                ...
                if not tp1_hit:
                    ...
                    if tp1_touch:
                        ...
                        entry_fee = entry_price * sz_half * TAKER_FEE
        ...
        if position == 0:
            ...
            # [MARKET v1] LONG: bot ≤ close ≤ top → 봉 close 가격으로 시장가 진입
            # 진입가 = c[i] (look-ahead 없음, taker fee)
            if htf_bull and len(long_top) > 0:
                best_k = -1
                best_bar = -1
                for k in range(len(long_top)):
                    if long_bar[k] < i and long_bot[k] <= c[i] <= long_top[k]:
                ...
                if best_k >= 0:
                    ...
                    if sl_dist > 0:
                        ...
                        # 시장가 진입 = 봉 close 시점 진입 → 진입봉 intrabar SL/TP 체크 불필요
                        entry_price = ep_i
            ...
            # [MARKET v1] SHORT: bot ≤ close ≤ top → 봉 close 가격으로 시장가 진입
            if htf_bear and len(short_top) > 0:
                best_k = -1
                best_bar = -1
                for k in range(len(short_top)):
                    if short_bar[k] < i and short_bot[k] <= c[i] <= short_top[k]:
