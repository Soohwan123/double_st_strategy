"""
Hyper Scalper Strategy Backtester V2 (EMA20 최적화 버전)
EMA20/100/200 + ADX + Retest (LONG & SHORT)

LONG 진입 조건:
1. bullTrend: close > EMA200 AND EMA20 > EMA100 > EMA200 (정배열)
2. strongTrend: ADX >= 45
3. hadLowBelowFast: 최근 15봉 내 저가가 EMA20 아래였던 적 있음
4. reclaimLong: 현재 종가 > EMA20
- 익절: 진입가 + ATR(14) * 10
- 손절: 최근 30봉 최저가

SHORT 진입 조건:
1. bearTrend: close < EMA200 AND EMA20 < EMA100 < EMA200 (역배열)
2. strongTrend: ADX >= 45
3. hadHighAboveFast: 최근 15봉 내 고가가 EMA20 위였던 적 있음
4. reclaimShort: 현재 종가 < EMA20
- 익절: 진입가 - ATR(14) * 3
- 손절: 최근 30봉 최고가

최적화 결과 (BTCUSDT 15m, 2020~2026):
  → 194,497% 수익, MDD 58.21%, WR 57.35%, 694 trades
"""
...
# ADX 설정
ADX_LENGTH = 14
ADX_THRESHOLD = 45.0  # 35 → 45

# Retest 설정
RETEST_LOOKBACK = 15  # 8 → 15

# 손절 설정
SL_LOOKBACK = 30  # 29 → 30
MAX_SL_DISTANCE = 0.035  # 3.5%

# ATR 설정
ATR_LENGTH = 14   # 10 → 14
TP_ATR_MULT_LONG = 10.0   # 4.1 → 10.0 (롱은 크게)
TP_ATR_MULT_SHORT = 3.0   # 4.1 → 3.0 (숏은 빨리)

# 수수료
MAKER_FEE = 0.0002
TAKER_FEE = 0.0005

fee_protection = False  # 수수료 보호 기능 활성화 여부
...
if __name__ == "__main__":
    # 데이터 로드
    print("Loading data...")
    df = pd.read_csv('historical_data/BTCUSDT_15m_futures.csv')
    ...
    # 결과 저장
    backtester.save_trades('trades_hyper_scalper_v2_ema20.csv')
    backtester.save_data_with_indicators('data_with_hyper_scalper_v2_ema20.csv')
