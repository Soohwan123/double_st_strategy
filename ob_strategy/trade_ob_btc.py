#!/usr/bin/env python3
"""OB Retest Live Trading — BTCUSDT 5m bt_01 v17 final (FVG sub account, lev 5 cap)"""
...
async def ipc_subscriber_task(strategy):
    """price_feed 의 ZMQ PUB 에서 BTCUSDT 시세 수신 → strategy 에 전달.

    bt_01 v17 OB swap (5m, HTF on): kline_5m + kline_1h + trade 구독.
    """
    subscriber = IPCSubscriber(
        symbol=SYMBOL,
        on_kline_main=strategy.on_candle_close,   # 5m kline
        on_kline_1h=strategy.on_htf_kline,         # HTF 1h EMA200 (use_htf=True)
        on_tick=strategy.on_tick,
        kline_interval='5m',                       # bt_01 = 5m
        logger=log_handler,
        ...
        try:
            ...
            if strategy.position.has_position() and pos_info is None:
                ...
                if exit_type == 'TP' and exit_price:
                    ...
                    logger.warning("TP/SL 확인 불가 — actual PnL 조회 (entry_order_id + startTime, BTCUSDT 만)")
                    pnl_data = None
                    ...
                    if pnl_data and pnl_data.get('net_pnl', 0) != 0:
                        ...
                        logger.warning(f"자본금: ${old:.2f} → ${strategy.capital:.2f}")
                    else:
...
async def main():
    logger = log_handler
    logger.info("=" * 70)
    logger.info(f"OB Retest Live Trading 시작 ({SYMBOL}) — bt_01 v17 final")
    logger.info("=" * 70)
