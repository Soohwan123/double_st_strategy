...
GRID_LEVELS = 5  # 5단계 (실제 진입은 4단계까지, 5단계 가격은 손절 트리거)
MAX_ENTRY_LEVEL = 4  # 최대 진입 레벨
# 진입 비율
ENTRY_RATIOS = [0.05, 0.10, 0.30, 0.55]  # 5%, 10%, 30%, 55%

GRID_INTERVAL_PCT = GRID_RANGE_PCT / GRID_LEVELS  # 1% 간격
...
class GridMartingaleBacktesterV3:
    ...
    def partial_close_and_reset_grid(self, exit_price: float, timestamp):
        ...
        # 그리드 기준가 역산
        # LONG: level1_price = grid_center * (1 - GRID_INTERVAL_PCT)
        #       → grid_center = level1_price / (1 - GRID_INTERVAL_PCT)
        # SHORT: level1_price = grid_center * (1 + GRID_INTERVAL_PCT)
        #        → grid_center = level1_price / (1 + GRID_INTERVAL_PCT)
        if self.position == 'LONG':
            self.grid_center = new_level1_price / (1 - GRID_INTERVAL_PCT)
        else:  # SHORT
            self.grid_center = new_level1_price / (1 + GRID_INTERVAL_PCT)
    ...
    def close_position(self, exit_price: float, reason: str, timestamp, is_market: bool = False):
        ...
        if self.current_level >= MAX_ENTRY_LEVEL:
            if self.position == 'LONG':
                sl_target_price = self.get_level_price(MAX_ENTRY_LEVEL, 'LONG')
            else:  # SHORT
                sl_target_price = self.get_level_price(MAX_ENTRY_LEVEL, 'SHORT')
    ...
    def run(self):
        ...
        print(f"Grid Interval: {GRID_INTERVAL_PCT*100:.2f}%")
        print(f"Entry Levels: {MAX_ENTRY_LEVEL} (Level 5 = SL trigger)")
        ...
        print(f"Stop Loss: Level 5 price touch")
